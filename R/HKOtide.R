...
#'
#' @examples HKOtide()
HKOtide = function(ETime = Sys.time(), DDays = 7, STime = NA, lan = "e", listfail = F){
  hkw_lib()
  ...
  #List URL and DIR
  Tide_URL = data.frame(Num = 1:(288*DDays))
  Tide_URL = subset(Tide_URL, Num != 0)
  Tide_URL$Time = ISOdate(Time_Spl$Year[2], Time_Spl$Month[2], Time_Spl$Day[2], Time_Spl$Hour[2], Time_Spl$Min[2], 0, tz = "HongKong") - minutes((Tide_URL$Num - 1)*5)
  Tide_URL$Time_N = Tide_URL$Time - minutes(5)
  ...
  Tide_URL$URL  = paste0("https://api.data.gov.hk/v1/historical-archive/get-file?url=https%3A%2F%2Fdata.weather.gov.hk%2FweatherAPI%2Fhko_data%2Ftide%2FALL_",
                         lan_HKG, ".csv&time=", Tide_URL$Date,"-", Tide_URL$Hour)
  Tide_URL$DIR  = paste0(getwd(), "/Tide", toupper(lan), "/", Tide_URL$Date_N, "/",
                         "Tide", toupper(lan), "-", Tide_URL$Date_N, "-", Tide_URL$Hour_N, ".csv")
  hkw_dir.cre(pri = c("/Tide", toupper(lan)), sec = Tide_URL$Date_N)
