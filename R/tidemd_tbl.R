#' Orders MDtide results (v2)
#'
#' Tidies up MDtide csv files. Creates a list of files and names
...
#' @export
#'
#' @examples tidemd_tbl(getwd())
tidemd_tbl = function(DIR, write = T){
  hkweather::hkw_lib()
  ...
  for(i in 1:nrow(df_name_list)){
    ...
    df_org = df_org %>%
      relocate(Min_00, Min_10, Min_20,
               Min_30, Min_40, Min_50, .after = Date) %>%
      arrange(Date)
    if(write == T){
