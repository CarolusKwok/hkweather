...
#' @param DTime The starting time of images to be downloaded, starting from the latest time. Only accepts POSIXct (tip: create POSIXct via ISOdatetime).
...
HKOsatellite = function(magn = 8, type = "dc", DDays = 3.5, DTime = Sys.time(), listfail = F){
  ...
  Time_Spl = data.frame(Time  = c("Now", "Sat"),
                        ...
                        Day   = day(DTime),
                        Hour  = hour(DTime),
                        Min   = c(minute(DTime), minute(DTime)-minute(DTime)%%10))
  Sat_URL          = data.frame(Num = seq(1:(144*DDays)))
  Sat_URL$Time     = ISOdate(Time_Spl$Year[2], Time_Spl$Month[2], Time_Spl$Day[2], Time_Spl$Hour[2], Time_Spl$Min[2], 0, tz = "") - minutes((Sat_URL$Num - 1)*10)
  Sat_URL$Time_UTC = with_tz(Sat_URL$Time, tzone = "UTC")
  Sat_URL$Date     = paste(sprintf("%04d", year( Sat_URL$Time)),
                           sprintf("%02d", month(Sat_URL$Time)),
                           sprintf("%02d", day(  Sat_URL$Time)),
                           sep = "")
  Sat_URL$URL      = paste("https://www.hko.gov.hk/wxinfo/intersat/satellite/image/images/h8L_",
                           type, "_x", magn, "M_",
                           sprintf("%04d", year(  Sat_URL$Time_UTC)),
                           sprintf("%02d", month( Sat_URL$Time_UTC)),
                           sprintf("%02d", day(   Sat_URL$Time_UTC)),
                           sprintf("%02d", hour(  Sat_URL$Time_UTC)),
                           sprintf("%02d", minute(Sat_URL$Time_UTC)),
                           "00.png", sep = "")
  Sat_URL$DIR      = paste(getwd(), "/Sat", sprintf("%02d", magn), type, "/", Sat_URL$Date, "/",
                           magn, type, "-",
                           sprintf("%04d", year(  Sat_URL$Time)),
                           sprintf("%02d", month( Sat_URL$Time)),
                           sprintf("%02d", day(   Sat_URL$Time)), "-",
                           sprintf("%02d", hour(  Sat_URL$Time)),
                           sprintf("%02d", minute(Sat_URL$Time)),
                           ".png", sep = "")
