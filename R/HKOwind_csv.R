...
#' @export
#'
#' @examples HKOwind_csv()
HKOwind_csv = function(ETime = Sys.time(), DDays = 7, STime = NA, listfail = F){
  hkw_lib()
  ...
  if(!is.na(STime)){
    DDays = as.double(difftime(Time_Spl$ISO[2], Time_Spl$ISO[3], units = "days"))
  }
  #List URL and DIR
  ...
  ""
  Tide_URL$URL  = paste0("https://api.data.gov.hk/v1/historical-archive/get-file?url=https%3A%2F%2Fdata.weather.gov.hk%2FweatherAPI%2Fhko_data%2Fregional-weather%2Flatest_10min_wind.csv&time=", Tide_URL$Date,"-", Tide_URL$Hour)
  Tide_URL$DIR  = paste0(getwd(), "/Wind & Gust", "/", Tide_URL$Date_N, "/",
                         "WindGust", "-", Tide_URL$Date_N, "-", Tide_URL$Hour_N, ".csv")
  hkw_dir.cre(pri = c("/Wind & Gust"), sec = Tide_URL$Date_N)
