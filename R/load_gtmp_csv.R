#' Download HKO grass temperature information
#'
...
#' @export
#'
#' @examples load_gtmp_csv()
load_gtmp_csv = function(ETime = Sys.time(), DDays = 7, STime = NA, lan = "en", listfail = F){
  hkweather::hkw_lib()
  #Test input
  ...
  URL = URL %>%
    filter(as.numeric(Time_n_p) >= 1700 | as.numeric(Time_n_p) <= 0800)

  URL$URL          = paste0("https://api.data.gov.hk/v1/historical-archive/get-file?url=https%3A%2F%2Fdata.weather.gov.hk%2FweatherAPI%2Fhko_data%2Fregional-weather%2Flatest_1min_grass",
  ...
  #"https://api.data.gov.hk/v1/historical-archive/get-file?url=https%3A%2F%2Fdata.weather.gov.hk%2FweatherAPI%2Fhko_data%2Fregional-weather%2Flatest_1min_grass_sc.csv&time=20220817-0000"#
